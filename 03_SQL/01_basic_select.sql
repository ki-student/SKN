...
select emp_name, salary, salary*12 as '연봉' from emp;
...
select emp_id, emp_name, salary, comm_pct, salary*comm_pct as '급여*커미션' from emp;
...
select emp_id, emp_name, job from emp where job = 'IT_PROG' or job = 'ST_MAN';
...
select emp_id, emp_name from emp where emp_name like '%!%%' escape '!';
...
-- select emp_id, emp_name, hire_date from emp where hire_date between '2004-01-01' and '2004-12-31';
...
select * from emp where dept_name = 'Sales' and job = 'SA_MAN' and salary <= 13000;
...
-- select emp_id, emp_name, job, dept_name from emp where job like '%MAN%' and (dept_name = 'Marketing' or dept_name = 'Sales');
...
select emp_id, emp_name, job, salary from emp order by job, salary desc;

-- 부서명을 부서명(dept_name)의 오름차순으로 정렬해 조회하시오.
select dept_name from emp order by dept_name;

-- 급여(salary)가 $5,000을 넘는 직원의 ID(emp_id), 이름(emp_name), 급여(salary)를 급여가 높은 순서부터 조회
select emp_id, emp_name, salary from emp where salary > 5000 order by salary desc;
-- select emp_id, emp_name, salary from emp where salary > 5000 order by 3 desc;
