...
select upper(emp_name) as '대문자', lower(emp_name) as '소문자', char_length(emp_name) as '글자수' from emp;
...
select emp_id, emp_name, salary, ceil(salary*1.15) as 'SAL_RAISE' from emp;


-- 위의 SQL문에서 인상 급여(sal_raise)와 급여(salary) 간의 차액을 추가로 조회 
-- (직원ID(emp_id), 이름(emp_name), 15% 인상급여, 인상된 급여와 기존 급여(salary)와 차액)
select emp_id, emp_name, salary, ceil(salary*1.15) as 'SAL_RAISE', ceil(salary*1.15-salary) as '차액' from emp;
...
-- round(val,-1) -> 음수로 작성 시 십의자리부터 역순으로 -1, -2, ...
...
select dayofweek(curdate()); -- 1이면 일요일

-- 시간 타입에서 시 분 초 조회
select curtime(), hour(curtime()), minute(curtime()), second(curtime());

-- 특정 기간 만큼 전,후의 일시를 조회
select curdate(), adddate(curdate(), interval 3 year), adddate(curdate(), interval 30 day), subdate(curdate(), interval 3 year);

-- EMP 테이블에서 부서이름(dept_name)이 'IT'인 직원들의 '입사일(hire_date)로 부터 10일전', 입사일, '입사일로 부터 10일 후' 의 날짜를 조회. 
select hire_date, subdate(hire_date, interval 10 day), adddate(hire_date, interval 10 day) from emp where dept_name = 'IT';

-- ID(emp_id)가 200인 직원의 이름(emp_name), 입사일(hire_date)를 조회. 입사일은 yyyy년 mm월 dd일 형식으로 출력.
select emp_name, date_format(hire_date, '%Y년 %m월 %d일') from emp where emp_id = 200;
-- %y 소문자로 하면 yy 형식
-- %d-%m-%Y 형식도 가능

--  각 직원의 이름(emp_name), 근무 개월수 (입사일에서 현재까지의 달 수)를 계산하여 조회. 근무개월수 내림차순으로 정렬.
select emp_name, hire_date, timestampdiff(month, hire_date, curdate()) from emp order by 3 desc;
...
select emp_id, emp_name, job, ifnull(dept_name, '부서미배치') as dept_name from emp order by dept_name;

-- EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 급여(salary), 커미션 (salary * comm_pct)을 조회. 커미션이 없는 직원은 0이 조회되록 한다.
select emp_id, emp_name, salary, ifnull(salary*comm_pct, 0) from emp;

-- comm_pct가 없으면 '커미션 없음', 있으면 '커미션 있음' 출력
...
***********************************************/
-- 시간을 정수형태로 변환
select now(), cast(now() as signed);
select cast(now() as char);

select convert(now(), signed), convert(now(), char);

-- 숫자를 날짜로 변환
select convert(20250404, date), convert(122345, time);
...
select emp_id, emp_name, job, case job when 'AD_PRES' then '대표'
...
select dept_name, salary, case dept_name when 'IT' then salary*0.1
...
select emp_id, emp_name, salary, case when salary < 5000 then salary*0.3
									  when salary between 5000 and 9999 then salary*0.2
                                      when salary >= 10000 then salary*0.1
                                      end as '급여 인상율'
from emp;
-- then에 연산자 사용 가능 => 함수 사용 가능