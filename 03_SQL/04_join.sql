...
select e.emp_id, e.emp_name, e.salary, e.comm_pct, d.dept_name, d.loc from emp e join dept d on e.dept_id = d.dept_id
...
select d.dept_id, d.dept_name, d.loc, e.emp_name from dept d join emp e on d.dept_id = e.dept_id where d.dept_id = 30;
...
select d.dept_name, d.dept_id, avg(e.salary) as '평균급여' from dept d join emp e on d.dept_id = e.dept_id
group by d.dept_id, d.dept_name order by 3 desc;

-- 직원의 ID(emp.emp_id), 이름(emp.emp_name), 업무명(job.job_title), 급여(emp.salary), 급여등급(salary_grade.grade), 소속부서명(dept.dept_name)을 조회.
select e.emp_id, e.emp_name, j.job_title, e.salary, concat(s.grade,'등급'), d.dept_name
from emp e join job j on e.job_id = j.job_id
join dept d on e.dept_id = d.dept_id join salary_grade s on e.salary between s.low_sal and s.high_sal
order by 1;
...
select emp_name as '상사이름' from emp m where m.emp_id = 100;
...
from emp e left join dept d on e.dept_id = d.dept_id order by 4;
...
select e.emp_id, e.emp_name, e.dept_id, d.dept_name ,d.loc from emp e  left join dept d on e.dept_id = d.dept_id and d.dept_id = 80;
...
select d.dept_id, ifnull(d.dept_name, 0), count(emp_id) as '직원수' from dept d left join emp e on d.dept_id = e.dept_id
group by d.dept_id, d.dept_name order by 3 desc;

-- count(*) 시 null 값도 같이 count -> emp_id(PK) 지정 후 count 진행
...
select e.emp_id, e.emp_name, m.emp_name as '상사이름', date_format(e.hire_date, '%Y/%m/%d') as 'hire_date'
from emp e left join emp m on e.mgr_id = m.mgr_id where e.dept_id = 90;
...
select e.emp_id, e.emp_name, j.job_title, e.salary, e.hire_date, m.emp_name as '상사이름', m.hire_date as '상사 입사일', d.dept_name, d.loc
from emp e left join job j on e.job_id = j.job_id left join emp m on e.mgr_id = m.mgr_id left join dept d on e.dept_id = d.dept_id
