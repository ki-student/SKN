...
select sum(salary) as '총합계', avg(salary) as '평균', min(salary) as '최소값', max(salary) as '최대값', stddev(salary) as '표준편차',
variance(salary) as '분산', count(*) as '총 직원수' from emp;

-- EMP 테이블에서 가장 최근 입사일(hire_date)과 가장 오래된 입사일을 조회
select max(hire_date) as '최근 입사일', min(hire_date) as '오래된 입사일' from emp;
...
select (max(salary) - min(salary)) as '차액' from emp;

-- 가장 긴 직원 이름(emp_name)이 몇글자 인지 조회.
select max(length(emp_name)) as '가장 긴 이름의 직원' from emp;
...
select sum(salary) as '총합계', avg(salary) as '평균', min(salary) as '최소값', max(salary) as '최대값', stddev(salary) as '표준편차',
variance(salary) as '분산', count(*) as '총 직원수' from emp group by job;
...
select job, count(*) as '직원수' from emp where dept_name in('Sales','Purchasing') group by job order by 2 desc;

-- 부서(dept_name), 업무(job) 별 최대, 평균급여(salary)를 조회.
select dept_name, job, max(salary) as '최대급여', avg(salary) as '평균급여' from emp group by dept_name, job;

-- 급여(salary) 범위별 직원수를 출력. 급여 범위는 10000 미만,  10000이상 두 범주.
select if(salary < 10000, '만 미만', '만 이상'), count(*) as '직원수' from emp group by if(salary < 10000, '만 미만', '만 이상');

-- 부서명(dept_name), 업무(job)별 직원수, 최고급여(salary)를 조회. 부서이름으로 오름차순 정렬.
select count(dept_name), count(job), max(salary) from emp order by dept_name;

-- 입사년도와(hire_date) 업무(job)가 같은 직원들의 평균 급여(salary)을 조회
select year(hire_date), avg(salary) from emp group by year(hire_date), job;

-- 부서별(dept_name) 직원수 조회하는데 부서명(dept_name)이 null인 것은 제외하고 조회.
select count(*) from emp group by dept_name having dept_name is not null;
...
select dept_name, avg(salary) as '평균 급여' from emp group by dept_name having count(*) >= 10;

-- 20명 이상이 입사한 년도와 (그 해에) 입사한 직원수를 조회.
select year(hire_date), count(*) from emp group by year(hire_date) having count(*) >= 20;

-- 평균 급여가(salary) $5000 이상인 부서의 이름(dept_name)과 평균 급여(salary), 직원수를 조회
select dept_name, avg(salary), count(*) from emp group by dept_name having avg(salary) >= 5000;

-- 평균급여가 $5,000 이상이고 총급여가 $50,000 이상인 부서의 부서명(dept_name), 평균급여와 총급여를 조회
select dept_name, avg(salary), sum(salary) from emp group by 1 having avg(salary) >= 5000 and sum(salary) >= 10000;

--  커미션이 있는 직원들의 입사년도별 평균 급여를 조회. 단 평균 급여가 $9,000 이상인 년도분만 조회.
select year(hire_date), avg(salary) as '평균 급여' from emp where comm_pct is not null group by year(hire_date) having avg(salary) >= 9000;
